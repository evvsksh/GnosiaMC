...
execute if entity @s[tag=!cantClaim] if entity @e[tag=playerDummy,tag=guard,tag=!checkForOtherGuard,tag=!dead] if score showRoleActionbar gameStatus matches 1 run execute as @e[tag=playerDummy,tag=guard,tag=!checkForOtherGuard,tag=!dead] run function gnosia:log/event/claim_guard
