scoreboard players operation tempMaxGamesSpectated temp > @a consecutiveGamesSpectated

execute as @a if score @s consecutiveGamesSpectated = tempMaxGamesSpectated temp run tag @s add gameCandidate
...
execute if entity @a[tag=!crew] unless score tempCrewCount temp matches 15 run function gnosia:phase/start/determine_players