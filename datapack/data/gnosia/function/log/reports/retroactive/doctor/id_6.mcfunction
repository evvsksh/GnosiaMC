...
execute unless entity @s[tag=invalid_target] run data remove block -300 88 -1610 Items[{Slot:23b}].components."minecraft:custom_data".doctorList[0]
execute unless entity @s[tag=invalid_target] if data block -300 88 -1610 Items[{Slot:23b}].components."minecraft:custom_data".doctorList[0] run function gnosia:log/reports/generate/retroactive/book/doctor/distribute
